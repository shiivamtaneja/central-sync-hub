...
const SignIn = ({ token }: { token: string }) => {
  ...
  const form = useForm<z.infer<typeof invitationSignInSchema>>({
    resolver: zodResolver(invitationSignInSchema),
    defaultValues: {
      email: '',
      token
    }
  ...
  const onSubmit = async (values: z.infer<typeof invitationSignInSchema>) => {
    try {
      ...
      if (response.ok) {
        if (response.status === 200) {
          ...
          router.replace(`/invite/${token}/verify`);
        }
      } else {
  ...
  return (
    <div className="border rounded-xl p-6 shadow-lg w-96 mx-3 flex flex-col space-y-4">
      ...
      <div className="flex items-center text-sm gap-1">
        <p>No account?</p>
        <Link href={`/invite/${token}/signup`} className="text-blue-600">Sign up</Link>
      </div>
    </div>
  );
};
