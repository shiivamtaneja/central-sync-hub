...
const SignUp = ({ token }: { token: string }) => {
  const router = useRouter();

  const setEmail = emailVerificationStore((state) => state.set);

  const form = useForm<z.infer<typeof invitationSignUpSchema>>({
  ...
  const onSubmit = async (values: z.infer<typeof invitationSignUpSchema>) => {
    try {
      ...
      if (response.ok) {
        if (response.status === 201) {
          ...
          router.replace(`/invite/${token}/signup/verify`);
        }
      } else {
